...
- **The lightbox** ("the photos page") is the one place that shows everything: Name, Date · Location, Category · Credit, written **History** (`.photo-lightbox-history`, only shown when `photo.history` is set), Ref, map/document/story-page actions, and a "Know more about this photo?" submission section.
...
**Admin: `admin/index.html` + `admin/config.yml`** is [Decap CMS](https://decapcms.org/), backed by `git-gateway` (Netlify Identity for login, saves commit straight to `main` on GitHub, no custom backend code). Three collections: `pages` (the story-page blocks editor described above), `photos` (folder collection over `data/photos/`, see above — `create`/`delete` both off, since new photos still have to come in via Drive to get resized/hosted), and `map_pins` (edits `data/map-pins.json`, including a `relation` field to pick a `pages` entry for `pageSlug`). `publish_mode: simple` — no draft/review step, matching the rest of the site's single-trusted-uploader approach; switch to `editorial_workflow` in `config.yml` if that ever needs to change (e.g. more admins added).
