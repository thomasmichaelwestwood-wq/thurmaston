...
- **Ordinary photos currently need their `pageSlug`, `location`, and `history` hand-set** in `data/photos.json` (no CMS UI for editing `data/photos.json` yet — see below) — or, more practically, give that photo a proper entry in `data/map-pins.json` instead (reusing its existing `photoSrc`/`photoId` and re-pasting its coordinates as a `coords` string), which also gets it a name, a description, a `location`, and a `pageSlug` field all editable through the admin, and automatically suppresses the auto-generated version of that same pin (matched by `photoId`). This is also the answer for a photo that didn't get coordinates at upload time — add/fix them here rather than needing to re-upload.
...
Two new optional fields on **both** `data/photos.json` entries and `data/map-pins.json` pins:
- `location` — free text, e.g. `"Garden Street"`. Not the same as `category` (Streets & Buildings etc.) — this is an actual place name, filled in by hand as it becomes known. Curated pins can set it via the Map Pins admin form; ordinary photos need it hand-set in `data/photos.json` (same gap as `pageSlug`, see below).
- `history` — **`data/photos.json` only** (curated pins already have `description` serving the same role for their popup). A longer written paragraph, shown in the lightbox beneath the meta line. Optional — most photos won't have one yet.
